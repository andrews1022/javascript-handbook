...
}).map(function (toDoThing) { // You can also add on additional methods, like .map()
...
const redTeamMembersArrow = teamArray.filter((user) => {
	if (user.team === 'red') {
		return true;
	}
})
...
const filteredPair = srFees.filter((pair) => {
	return (
		pair.verticalNum == checkedBusinessBoxes && pair.horizontalNum == checkekRegionBoxes
	);
});
